    ...
    return {
      ...
      clueRules: {
        phone: [
          ...
          {validator: this.checkPhone, message: '该手机号已被录入', trigger: 'blur'}
        ],
        fullName : [
  ...
  methods: {
    ...
    addClueSubmit() {
      this.$refs.clueRefForm.validate(isValid => {
        ...
        if (isValid) {
          for (let data in this.clueQuery) {
            if (this.clueQuery[data]) {
              formData.append(data, this.clueQuery[data]);
            }
          }
          if (this.clueQuery.id > 0) {  //编辑活动
            doPut('/api/clue', formData).then(resp => {
              if (resp.data.code === 200) {
                messageTip('编辑成功', 'success');
                this.$router.push('/dashboard/clue');
              } else {
                messageTip('编辑失败', 'warning');
              }
            })
          } else {
      ...
      if (id) {
        doGet(`/api/clue/detail/${id}`, {}).then(resp => {
          if (resp.data.code === 200) {
            ...
            // 查询市场活动
            let activityId = this.clueQuery.activityId;
            doGet(`/api/activity/${activityId}`, {}).then(resp => {
              if (resp.data.code === 200) {
                this.activityOptions.push(resp.data.data);
              }
            })
          }
        })
...
<template>
  <el-form
    ...
    <el-form-item>
      ...
      <el-button type="success" plain @click="$router.go(-1)">返 回</el-button>
    </el-form-item>
  </el-form>
</template>
